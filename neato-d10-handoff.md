...
**✅ DONE this session (2026-08-06, session 4): battery BOM fully sourced.** BMS + charger + fuse ordered; voltage sense = build `10k+2k` divider from Elegoo kit; pack disconnect = salvage OEM connector; bench safety = nested terracotta pots on concrete; hookup wire = salvage stranded PC-PSU peripheral cables (keep PSU whole as step-2 12V bench supply). Nothing confirmed left to buy — buy-triggers only if a salvage fails at the BMS-swap bench session (fallbacks recorded: XT60 B0CF89QJGR, 14 AWG silicone B0CZRG3QMK). First power-spine Mermaid diagram drawn → topology questions queued below.

1. **[TOP PRIORITY — doable NOW, newly unblocked] Build step 2 — encoders → odometry.** Wire a **real** Neato wheel motor + a **DRV8871** (⚠️ the OEM pack is dead/latched — power the DRV8871 VIN from a **bench PSU / 12–15 V ≥2 A supply**, NOT the L293D which can't take the ~2 A stall). **Bench PSU = the salvaged PC ATX PSU's 12V rail** (jump green PS_ON → black to power on; plenty of amps) — free and already on hand. Drive from the existing serial protocol, then read the **quadrature encoder (A/B)** into the ESP32 for distance + direction. Fat, easy wires — no fiddly connectors. This is the real-hardware version of what step 1 proved on a stand-in, and the drivers are finally here. **Do this first.**
